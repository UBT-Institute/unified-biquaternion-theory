...
| konečná identita Abelovy sumace | klasická přesná identita | kontrola komplexní aritmetiky v čistém Pythonu bez závislostí, tentýž artefakt | porovnává oba konečné výrazy v ne­reálném \(s\) | pouze výběrové numerické vyhodnocení | `LEAN-PASS` — diskrétní identita pro libovolné váhy ověřena; integrální identita a nekonečná limita zůstávají neformalizované; viz `mobius_abel_lean.cs.md` |
