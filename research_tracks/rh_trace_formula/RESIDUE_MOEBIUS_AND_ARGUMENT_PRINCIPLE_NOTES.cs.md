...
| konečná identita Abelovy sumace | klasická přesná identita | kontrola komplexní aritmetiky v čistém Pythonu bez závislostí, tentýž artefakt | porovnává oba konečné výrazy v ne­reálném \(s\) | pouze výběrové numerické vyhodnocení | `LEAN-PASS` — diskrétní identita pro libovolné váhy ověřena; integrální identita a použití Möbiovy mocninné meze zůstávají neformalizované; podmíněná limita v přirozeném pořadí: `abel_limit_and_cancellation.cs.md`; viz `mobius_abel_lean.cs.md` |
